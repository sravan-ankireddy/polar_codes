...
if bipartite==1
    ...
    [H_BP_tot] = polar2bipartite(N,A_BP_tot,1);
...
%     del_1 = 50; del_2 = 3;
    ...
    H_BP_tot = pruneGraph(H_BP_tot,N);
    H_BP = pruneGraph(H_BP,N);
else
    H_BP_tot = createDensePolarH(N,A_BP_tot);
    Hdense = H_BP_tot; 
end

[ParRI_BP, ParCI_par] = find(H_BP == 1);
ParI_BP = [ParRI_BP, ParCI_par];
...
H_tot = zeros(size(H_BP_tot,1)+num_par_eq, size(H_BP_tot,2)+num_par_eq);
H_tot(1:size(H_BP_tot,1),1:size(H_BP_tot,2)) = H_BP_tot;
H_tot(size(H_BP_tot,1)+1:end,end-N-num_par_eq+1:end) = H_par;
...
%         LLR_tot_hat = step(decoder_tot, Lch_ext_tot); xhat_tot=(-sign(LLR_tot_hat))/2+0.5;
        ...
        xhat_BP = ldpc_minsum_decode(H_BP,0,Lch_ext_BP,max_iter)';
        
        it = 0; done_BP = 1;
        L0 = LLR_BP_tot';
        L1 = Lch_ext_tot(1:end-num_par_eq)';
        L1(end-N+1:end) = LLR_BP_tot(1:N); % corresponds to polar decoder
        L2 = LLR_BP_tot'; % corresponds to par mat
        for it = 0:max_iter
            if (done_BP == 1)
                [xhat_tot,done_BP,L1_out,~,~] = ldpc_minsum_decode2(H_BP,it,0,L1,max_iter);
            else
                xhat_tot = xhat_tot((end-N+1):end);
                break;
            end
%             L1 = L1_out;
%             if (it == 0)
%                 L2(1:N) = L1_out(end-N+1:end);
%             else
%                 L2(1:N) = L0(1:N) + (L1_out(end-N+1:end) - L1(end-N+1:end)); 
%             end
            L2(1:N) = L1_out(end-N+1:end);
            [~,~,L2_out,~,~] = ldpc_minsum_decode2(H_par,it,0,L2,max_iter); xhat_tot = xhat_tot(1:N);
%             L1(end-N+1:end) = L0(1:N) + (L2_out(1:N) - L2(1:N));
             L1(end-N+1:end) = L2_out(1:N);
        end

%         xhat_tot = ldpc_minsum_decode_alternate(H_BP_tot,H_par,N,num_par_eq,0,LLR_BP_tot,max_iter)';
%         [xhat_tot,~,~,~] = ldpc_minsum_decode(H_tot,0,Lch_ext_tot,max_iter);
        xhat_tot = xhat_tot';        
        msg_hat_BP = xhat_BP(end-N+1:end); msg_hat_BP = polarTransform(msg_hat_BP, A_BP); msg_hat_BP = msg_hat_BP(A_BP);
        msg_hat_tot = xhat_tot(end-N+1:end); msg_hat_tot = polarTransform(msg_hat_tot, A_BP_tot); msg_hat_tot = msg_hat_tot(A_BP_tot);

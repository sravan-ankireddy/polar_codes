...
%     del = 50; mul = 3;
    del = 5; mul = 1;
    par_pos = 1:del:num_par_bits*del+1;
%     par_pos = randi([1 N], num_par_bits,1);
    for i = 1:num_par_eq
        H_par(i,par_pos+mul*i-1) = 1;
        H_par(i,N+i) = 1;
    end
...
[ParRI_par, ParCI_par] = find(H_par == 1);
ParI_par = [ParRI_par, ParCI_par];

% decoder_par = comm.LDPCDecoder('ParityCheckMatrix',ParI_par,'OutputValue','Whole codeword','DecisionMethod','Soft decision','MaximumIterationCount',max_iter,'NumIterationsOutputPort',1,'IterationTerminationCondition', 'Parity check satisfied');
...
EbN0dB_start = 1; EbN0dB_stop = 3.2;
...
BER_BP = zeros(1,nSam);
BER_BP_par = zeros(1,nSam);

% Vector to store Bit Error Rate
BLER_SSCD = zeros(1,nSam);
BLER_BP = zeros(1,nSam);
BLER_BP_par = zeros(1,nSam);
...
for i_E = 1:length(EbN0dB)
	...
	parfor i_m = 1:Nblocks
        ...
        % add parity
        x_BP = [x_BP; zeros(num_par_eq,1)];
        for i_par = 1:num_par_eq
            x_BP(N+i_par) = mod(sum(x_BP(par_pos+mul*i_par-1)),2);
        end
		...
		% Adding AWGN noise
        noise_SSC = sig * randn(size(x_SSC));
		y_SSC = x_SSC + noise_SSC;
        noise_BP = sig * randn(size(x_BP));
        y_BP = x_BP + noise_BP;
        ...
        Lch_ext((end-N+1):end) = LLR_BP(1:N);
        
        LLR_BP_hat = step(decoder, Lch_ext); xhat_BP=(-sign(LLR_BP_hat))/2+0.5;
        
%         [xhat_BP, LLR_BP_hat,~,~] = ldpc_minsum_decode(H,0,Lch_ext,max_iter); xhat_BP = xhat_BP'; 
        ...
        % LLR for parity
        LLR_par = zeros(size(H_par,2),1);
        
        LLR_par(1:N) = LLR_BP_hat(end-N+1:end); LLR_par(N+1:end) = LLR_BP(N+1:end);
        ...
        for i = 1:10
            ...
            [~,LLR_par_out,~,~] = ldpc_minsum_decode(H_par,0,LLR_par,max_iter);
            
%             whos Lch_ext; whos L0; whos LLR_par_out; whos LLR_par;
            
            Lch_ext((end-N+1):end) = L0(end-N+1:end) + LLR_par_out(1:N)' - LLR_par(1:N);
            
%             Lch_ext((end-N+1):end) = LLR_par_out(1:N)';
            
            LLR_BP_hat = step(decoder, Lch_ext); 
        
            LLR_par(1:N) = L0(end-N+1:end) + LLR_BP_hat(end-N+1:end) - Lch_ext((end-N+1):end) ;
            
%             LLR_par(1:N) = LLR_BP_hat(end-N+1:end);
% 
%             [~,LLR_par_out,~,~] = ldpc_minsum_decode(H_par,0,LLR_par,max_iter);
% 
%             Lch_ext((end-N+1):end) = LLR_par_out(1:N);
...
%         LLR_BP_hat = step(decoder, Lch_ext);
        ...
        xhat_BP_par=(-sign(LLR_BP_hat))/2+0.5;        
        
        msg_hat_BP_par = xhat_BP_par(end-N+1:end);
                
        msg_hat_BP_par = polarTransform(msg_hat_BP_par, A);
        
        msg_hat_BP_par = msg_hat_BP_par(A);
        
        % Checking for errors
        nberr_SSC = sum(xor(msg,msg_hat_SSC));
        ...
        nberr_BP_par = sum(xor(msg,msg_hat_BP_par));
        err_BP_par = err_BP_par + nberr_BP_par;
        if (nberr_BP_par > 0)
            fer_BP_par = fer_BP_par + 1;
        end
    ...
    BER_BP_par(1,i_E) = err_BP_par/(Nblocks_count*N);
    BLER_BP_par(1,i_E) = fer_BP_par/(Nblocks_count);
...
semilogy(EbN0dB,BER_BP_par,'--x');
semilogy(EbN0dB,BLER_BP_par,'--x');

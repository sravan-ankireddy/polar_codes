...
simparam.useSysEncoding=0;   %use systematic encoding
simparam.useMinSum=0;        %use minsum instead of inbuitlt
...
decoder = comm.LDPCDecoder('ParityCheckMatrix',ParI,'OutputValue','Whole codeword','DecisionMethod','Soft decision','MaximumIterationCount',simparam.iterMax,'NumIterationsOutputPort',1,'IterationTerminationCondition', 'Parity check satisfied');

for j=1:simparam.nbSNR
    disp(['evaluating SNRb=' num2str(simres.SNRb(j)) 'dB, ' num2str(j) '/' num2str(simparam.nbSNR)]);
    ...
    parfor i=1:simparam.nbCWs
        ...
        if simparam.useMinSum
            [xhat, iter, ~] = ldpc_minsum_decode(H,0,Lch_ext,simparam.iterMax);
        else
        	[LLRxhat, iter] = step(decoder, Lch_ext); xhat=(-sign(LLRxhat))/2+0.5;    %and decode
        end
...
if createPlots==1
    figure(1);
    semilogy(simres.SNRb,simres.BER);hold on;
    semilogy(simres.SNRb,simres.BLER,':');

...
BER_SCD = zeros(1,length(var_N));

% Vector to store Bit Error Rate
FER_SCD = zeros(1,length(var_N));
...
for i_E = 1:length(EbN0dB)
	...
	parfor i_m = 1:Nblocks
        ...
        % SC Decoding
        msg_hat = decode_SCD(LLR, N, info_check_vec, data_pos);
        
        % Checking for errors
        nberr = sum(xor(msg,msg_hat));
        err_SCD = err_SCD + nberr;
        if (nberr > 0)
            fer_SCD = fer_SCD + 1;
        end
    ...
    BER_SCD(1,i_E) = err_SCD/(Nblocks_count*N);
    FER_SCD(1,i_E) = fer_SCD/(Nblocks_count);
...
figure(1)
semilogy(EbN0dB,BER_SCD,'--*');
hold on;
semilogy(EbN0dB,FER_SCD,'-*');

legend('BER','FER');
str = sprintf('Plot for BER vs Eb/N0 for SCD');
title(str);
xlabel('Eb/N0 in dB');

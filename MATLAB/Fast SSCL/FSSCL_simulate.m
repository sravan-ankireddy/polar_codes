...
if (crc_len == 8)
    ...
    % CRC Detector
    crcDet = comm.CRCDetector([8 7 6 4 2 0]);
end
...
BER_SCL = zeros(1,length(var_N));

% Vector to store Bit Error Rate
FER_SCL = zeros(1,length(var_N));
...
for i_E = 1:length(EbN0dB)
	...
	parfor i_m = 1:Nblocks
        ...
        % SC Decoding
        [msg_hat,PM] = decode_FSSCL(LLR, N, l, info_check_vec, data_pos, node_type_mat);
        
        crc_err = zeros(1,l);
        for i_crc = 1:l
            [~, crc_err(i_crc)] = crcDet(transpose(msg_hat(i_crc,:)));
        end
    ...
    BER_SCL(1,i_E) = err_SCL/(Nblocks_count*(K-crc_len));
    FER_SCL(1,i_E) = fer_SCL/(Nblocks_count);
...
figure(1)
semilogy(EbN0dB,BER_SCL,'-s');
hold on;
semilogy(EbN0dB,FER_SCL,'-p');

...
BER_SSCD = zeros(1,length(var_N));

% Vector to store Bit Error Rate
BLER_SSCD = zeros(1,length(var_N));
...
for i_E = 1:length(EbN0dB)
	...
	parfor i_m = 1:Nblocks
        ...
        % SC Decoding
        msg_hat = decode_SSCD(LLR, N, node_type_mat, info_check_vec, data_pos);
        
        % Checking for errors
        nberr = sum(xor(msg,msg_hat));
        err_SSCD = err_SSCD + nberr;
        if (nberr > 0)
            fer_SSCD = fer_SSCD + 1;
        end
...
figure(1)
semilogy(EbN0dB,BER_SSCD,'--d');
hold on;
semilogy(EbN0dB,BLER_SSCD,'-d');

legend('BER','BLER');
str = sprintf('Plot for BER vs Eb/N0 for SSCD');
title(str);
xlabel('Eb/N0 in dB');

...
%             disp('zer');
        ...
        % Rate-1 node
        elseif (node_type_mat(depth+1,node*cur_len+1) == 201 && counter > log2(l) && done == 0 && ns(npos)~=2 )
        ...
        elseif (node_type_mat(depth+1,node*cur_len+1) == 301 && counter > log2(l) && done == 0 && ns(npos)~=2 )
            ...
            [~, min_pos] = min(abs(squeeze(Ln)));
            
            min_LLR = squeeze(Ln(:,1,min_pos));
            ...
            for i = 1:cur_len-1
                ...
                min_pos = min_pos(ind_ord_l); min_LLR = min_LLR(ind_ord_l);
            ...
            for i = 1:l
                par = mod(sum(squeeze(ucapn(i,1,end-1))),2);
                ucapn(i,1,[1:min_pos(i)-1 min_pos(i)+1:end]) = ucapn(i,1,1:end-1);
                ucapn(i,1,min_pos(i)) = par;
            end
            
            ucap(:,depth+1,node*cur_len+1:node*cur_len+cur_len) = ucapn;
            
            for i = 1:l
                if (ucapn(i,1,min_pos(i)) ~= (min_LLR(i) < 0))

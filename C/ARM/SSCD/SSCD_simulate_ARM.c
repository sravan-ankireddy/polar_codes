...
/* Code Parameters */
    ...
    find_node_type(node_type, K, N-K, N, N, 0, data_pos_sorted, frozen_pos_sorted);
...
/* Simulations */
    ...
    float noise[N];
    int i_noise = 0;
    for (i_noise = 0; i_noise < N; i_noise++)
    {
        noise[i_noise] = randn(0,1);
    }
    ...
    for (i_sig = 0; i_sig < num_EbN0dB; i_sig++)
    {
        ...
        for (i_num_sim = 0; i_num_sim < num_sim; i_num_sim++)
        {
            ...
            for (i_bpsk = 0; i_bpsk < N; i_bpsk++)
            {
                ...
                // y[i_bpsk] = x[i_bpsk] + sig*noise[i_bpsk];
            }

...
void decode_unrolled(int N, int K, int l, int *info_nodes, int *data_pos, int *node_type, int *LLR_Q, int *msg_cap)
{
    ...
    while (!(done == 1 && depth == -1)) //traverse till all bits are decoded and root node is reached again
    {
        ...
        if (depth == n && done == 0)
        {
            if ( info_nodes[node_type_ind] == 0 )
            {
                ...
                for (i_list = 0; i_list < l; i_list++)
                {
                    ...
                    ind_ord_mat[depth][l*node + i_list] = ind_ord_l[i_list];
                }
            ...
            else
            {
                if (counter > ln - 1)
                {
                    ...
                    for (i_list = 0; i_list < l; i_list++)
                    {
                        ...
                        ind_ord_mat[depth][l*node + i_list] = ind_ord[i_list]%l;
                    }
        ...
        /* SPC nodes */
        else if(node_type[node_type_ind + N*(n-depth)] == 301 && counter > ln -1 && done == 0 && ns[npos] != 2)
        {
            /* Incoming LLR Values and decisions*/
            int L_in[l*temp];
            int min_LLR_pos[l];
            ...
            for (i_list = 0; i_list < l; i_list++)
            {
                min_LLR_pos[i_list] = 0;
                min_LLR[i_list] = L[depth][i_list*N];

                for (i_N = 0; i_N < temp; i_N++)
                {
                    L_in[i_N + i_list*temp] = L[depth][i_N + i_list*N];

                    if ( absl_macro(L_in[i_N + i_list*temp]) < absl_macro(min_LLR[i_list]) )
                    {
                        min_LLR_pos[i_list] = i_N;
                        min_LLR[i_list] = L_in[i_N + i_list*temp];
                    }
                }
                L_in[min_LLR_pos[i_list] + i_list*temp] = 1000*L_in[min_LLR_pos[i_list] + i_list*temp];
            }
            ...
            for (i_NN = 0; i_NN < temp; i_NN++)
            {
                ...
                for (i_list = 0; i_list < l; i_list++)
                {
                    PM_temp[i_list] = PM[i_list];
                    PM_temp[i_list + l] = PM[i_list];
                    par_bit_copy[i_list] = par_bit[i_list];
                }
                ...
                for (i_list = 0; i_list < l; i_list++)
                {
                    ...
                    par_bit[i_list] = par_bit_copy[ind_ord_temp[i_list]%l];
                ...
                for (i_list = 0; i_list < l; i_list++)
                {
                    ...
                    par_bit[i_list] = par_bit[i_list]^codeword_temp[ind_ord_temp[i_list]];
            ...
            for (i_list = 0; i_list < l; i_list++)
            {
                beta[depth][min_LLR_pos[ind_ord[i_list]%l] + node_type_ind + i_list*N] = beta[depth][min_LLR_pos[ind_ord[i_list]%l] + node_type_ind + i_list*N] ^ par_bit[i_list];

                if ((1 - 2*beta[depth][min_LLR_pos[ind_ord[i_list]%l] + node_type_ind + i_list*N]) != sign_macro(min_LLR[ind_ord[i_list]%l]))
                {
                    PM[i_list] += absl_macro(min_LLR[ind_ord[i_list]%l]);
                }
            }
            ...
            // for (i_list = 0; i_list < l; i_list++)
            // {
            //     ind_ord_re[i_list] = i_list;
            // }

            // /* Sorting the Path Metrics  */
            // insertionSort(PM, ind_ord_re, l);

            // /* New ordering */
            // for (i_list = 0; i_list < l; i_list++)
            // {
            //     // ind_ord_mat[depth][node + i_list*N] = ind_ord[ind_ord_re[i_list]]%l;
            //     ind_ord_mat[depth][l*node + i_list] = ind_ord[ind_ord_re[i_list]]%l;
            // }
            ...
            //     for (i_N = 0; i_N < temp; i_N++)
            //     {
            //         beta[depth][i_N + node_type_ind + i_list*N] = beta_copy[i_N + ind_ord_re[i_list]*temp];
            //     }
            // }
        ...
        else if ( node_type[node_type_ind + N*(n-depth)] == 40 && done == 0 &&  ns[npos] != 2)
        {
            if ( counter > ln - 1)
            {
                ...
                for (i_list = 0; i_list < l; i_list++)
                {
                    ...
                    ind_ord_mat[depth][node*l + i_list] = ind_ord_temp[i_list]%l;
                }
            ...
            else
            {
                ...
                {
                    // ind_ord_mat[depth][node + i_list*N] = ind_ord_l[i_list];
                    ind_ord_mat[depth][node*l + i_list] = ind_ord_l[i_list];
                }
            ...
            else
            {
                ...
                if (ns[npos] == 1 && done == 0)
                {
                    ...
                    for (i_list = 0; i_list < l; i_list++)
                    {
                        ...
                        i_temp_ord = (ind_ord_mat[depth+1][2*l*node + i_list])*N;
                ...
                else
                {
                    ...
                    for (i_list = 0; i_list < l; i_list++)
                    {
                        ...
                        ind_ord_temp[i_list] = ind_ord_mat[depth + 1][2*l*node + i_list];

                        /* Order of surviving decoders from right */
                        // ind_ord_temp2[i_list] = ind_ord_mat[depth + 1][2*node + 1 + i_list*N];
                        ind_ord_temp2[i_list] = ind_ord_mat[depth + 1][(2*node + 1)*l + i_list];
                    }
                    ...
                    for (i_list = 0; i_list < l; i_list++)
                    {
                        ...
                        ind_ord_mat[depth][l*node + i_list] = ind_ord_temp[i_temp];
                    }
                    ...
                    for (i_list = 0; i_list < l; i_list++)
                    {
                        ...
                        i_temp_ord = node_type_ind + (ind_ord_mat[depth + 1][(2*node + 1)*l + i_list])*N;
